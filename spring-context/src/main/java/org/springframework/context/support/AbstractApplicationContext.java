...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void initApplicationEventMulticaster() {
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		if (beanFactory.containsLocalBean(APPLICATION_EVENT_MULTICASTER_BEAN_NAME)) { //如果存在应该是用户自己定义
			...
			this.applicationEventMulticaster = new SimpleApplicationEventMulticaster(beanFactory); //不存在则使用默认
	...
	protected void initLifecycleProcessor() {
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		if (beanFactory.containsLocalBean(LIFECYCLE_PROCESSOR_BEAN_NAME)) { //用户自定义的
			...
			DefaultLifecycleProcessor defaultProcessor = new DefaultLifecycleProcessor(); //spring默认的
	...
	protected void finishRefresh() {
		...
		// 这个是Spring5.0之后才有的方法
		// 表示清除一些resourceCaches,如doc说的  清除context级别的资源缓存，比如ASM的元数据
		clearResourceCaches();
